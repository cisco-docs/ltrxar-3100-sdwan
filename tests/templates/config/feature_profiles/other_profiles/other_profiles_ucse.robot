...
Default Tags    sdwan    config    feature_profiles    other_feature_profiles    ucse
...
Verify Feature Profiles Other Profiles {{ profile.name }} UCSE Feature {{ profile.ucse.name | default(defaults.sdwan.feature_profiles.other_profiles.usce.name) }}
    ...
    Should Be Equal Value Json List Length    ${other_ucse[0]}   $..interface    {{ profile.ucse.interfaces | default([]) | length }}    msg=interfaces length
