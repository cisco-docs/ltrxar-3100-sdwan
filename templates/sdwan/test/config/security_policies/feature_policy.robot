...
Get Feature Policy List(s)
    ...
    ${z}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/zonebasedfw
    Set Suite Variable    ${z}
    Log    ${z.json()}

    ${p}=    GET On Session    sdwan_manager    /dataservice/template/policy/list/zone
...
{% for firewall_policy in feature_policy.unified_firewall_policies | default([]) %}
   ${firewall_data}=    Get Value From Json    ${z.json()}    $.data[?(@.name=='{{ firewall_policy.firewall_policy }}')]
   Should Be Equal Value Json String    ${firewall_data}    $..name    {{ firewall_policy.firewall_policy }}    msg=firewall_policy.firewall_policy
   ${firewall_id}=    Get Value From Json    ${firewall_data}    $..definitionId
   ...
   ${u_fp_zones}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.definitionId=='${firewall_id[0]}')].entries[*]

{% for zp in firewall_policy.zones %}
   IF    '{{ zp.source_zone }}' == 'self_zone'
       ${src_zone_id}=    Set Variable    self
   ELSE
       ${src_zone_id}=    Get Value From Json    ${p.json()}    $.data[?(@.name=='{{ zp.source_zone }}')].listId
      ${src_zone_id}=    Set Variable    ${src_zone_id[0]}
   END
   IF    '{{ zp.destination_zone }}' == 'self_zone'
      ${dst_zone_id}=    Set Variable    self
   ELSE
      ${dst_zone_id}=    Get Value From Json    ${p.json()}    $.data[?(@.name=='{{ zp.destination_zone }}')].listId
      ${dst_zone_id}=    Set Variable    ${dst_zone_id[0]}
   END
