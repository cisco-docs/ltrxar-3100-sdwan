...
Default Tags    sdwan  config  site_lists
Resource        ../../sdwan_common.resource

{% if sdwan.policy_objects.site_lists is defined%}

*** Test Cases ***
Get Site List(s)
    ${r}=   GET On Session   sdwan_manager   /dataservice/template/policy/list/site
...
{% elif site.site_id_ranges is defined %}
    ...
    Should Be Equal Value Json List   ${r_id.json()}   $..siteId   ${id_list}   msg=site id ranges are
{% endif %}
