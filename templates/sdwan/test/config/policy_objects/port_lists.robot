...
*** Test cases ***
Get Port Lists(s)
    ${r}=   Get On Session   sdwan_manager   /dataservice/template/policy/list/port
    Log   ${r}
...
Check Number of Port Lists Entries(s)
    ...
    ${actual_port_lists}=    Get Value From Json    ${r.json()}    $.data[*]    msg=actual_port_lists
...
Verify Port List Values {{port_list.name}}
    ${Actual_plist_values}=    Get Value From Json    ${r.json()}    $.data[?(@.name=={{port_list.name}})].entries[*]["port"]
    ${yaml_plist_values}=   Create List   {{ port_list.ports | default([]) | join('   ') }}
    Log    ${yaml_plist_values}
    Lists Should Be Equal    ${Actual_plist_values}    ${yaml_plist_values}
