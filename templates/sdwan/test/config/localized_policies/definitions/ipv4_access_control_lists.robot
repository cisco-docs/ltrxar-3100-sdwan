...
Get IPv4 Access Control Lists
    ${r}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/acl
...
Verify Localized Policies IPv4 Access Control Lists {{ ipv4_acl.name }}
    ...
    Should Be Equal As Integers    ${sequence_length}    {{ ipv4_acl.sequences | default([]) | length }}    msg=sequences
...
{% for sequence in ipv4_acl.sequences | default([]) %}
    ...
    ${ddpl_id}=    Get Value From Json    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="destinationDataPrefixList")].ref
    IF    ${ddpl_id} == []
    ...
    ELSE
        ${ddpl_match_id}=   GET On Session   sdwan_manager   /dataservice/template/policy/list/dataprefix/${ddpl_id[0]}
...
{% endif %}
    ...
    ${sdpl_id}=    Get Value From Json    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="sourceDataPrefixList")].ref
    IF    ${sdpl_id} == []
    ...
    ELSE
        ${sdpl_match_id}=   GET On Session   sdwan_manager   /dataservice/template/policy/list/dataprefix/${sdpl_id[0]}
...
{% endif %}
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="dscp")].value    {{ sequence.actions.dscp | default("not_defined") }}    msg=dscp
