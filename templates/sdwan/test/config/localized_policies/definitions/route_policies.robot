...
{% for sequence in route_policy.sequences | default([]) %}
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].sequenceName    {{ sequence.name | default("Route") }}    msg=name
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="localPreference")].value    {{ sequence.match_criterias.bgp_local_preference | default("not_defined") }}    msg=bgp local preference
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="metric")].value    {{ sequence.match_criterias.metric | default("not_defined") }}    msg=metric
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="ompTag")].value    {{ sequence.match_criterias.omp_tag | default("not_defined") }}    msg=omp tag
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="origin")].value    {{ sequence.match_criterias.origin | default("not_defined") }}    msg=origin
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="ospfTag")].value    {{ sequence.match_criterias.ospf_tag | default("not_defined") }}    msg=ospf tag
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@.field=="peer")].value    {{ sequence.match_criterias.peer | default("not_defined") }}    msg=peer
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="atomicAggregate")].value    {{ ((sequence.actions | default({})).atomic_aggregate | default("not_defined") | lower) if (sequence.actions | default({})).atomic_aggregate | default("not_defined") != "not_defined" else "not_defined" }}    msg=atomic aggregate

{% if (sequence.actions | default({})).communities | default("not_defined") == 'not_defined' %}
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="community")].value    {{ (sequence.actions | default({})).communities | default("not_defined") }}    msg=no communities defined
{% else %}
...
{% endif %}
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="community")].vipVariableName    {{ (sequence.actions | default({})).community_variable | default("not_defined") }}    msg=community variable

    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="communityAdditive")].value    {{ ((sequence.actions | default({})).community_additive | default("not_defined") | lower) if (sequence.actions | default({})).community_additive | default("not_defined") != "not_defined" else "not_defined" }}    msg=community additive

{% if (sequence.actions | default({})).exclude_as_paths | default("not_defined") == 'not_defined' %}
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="asPath")].value.exclude    {{ (sequence.actions | default({})).exclude_as_paths | default("not_defined") }}    msg=no exclude as path defined
{% else %}
...
{% endif %}
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="localPreference")].value    {{ (sequence.actions | default({})).local_preference | default("not_defined") }}    msg=local preference
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="metric")].value    {{ (sequence.actions | default({})).metric | default("not_defined") }}    msg=metric
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="metricType")].value    {{ (sequence.actions | default({})).metric_type | default("not_defined") }}    msg=metric type
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="nextHop")].value    {{ (sequence.actions | default({})).next_hop | default("not_defined") }}    msg=next hop
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="ompTag")].value     {{ (sequence.actions | default({})).omp_tag | default("not_defined") }}    msg=omp tag
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="origin")].value     {{ (sequence.actions | default({})).origin | default("not_defined") }}    msg=origin
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="originator")].value     {{ (sequence.actions | default({})).originator | default("not_defined") }}    msg=originator
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="ospfTag")].value     {{ (sequence.actions | default({})).ospf_tag | default("not_defined") }}    msg=ospf tag

{% if (sequence.actions | default({})).prepend_as_paths | default("not_defined") == 'not_defined' %}
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="asPath")].value.prepend    {{ (sequence.actions | default({})).prepend_as_paths | default("not_defined") }}    msg=no prepend as path defined
{% else %}
...
{% endif %}
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $..sequences[{{loop.index0}}].actions[?(@.type=="set")].parameter[?(@.field=="weight")].value    {{ (sequence.actions | default({})).weight | default("not_defined") }}    msg=weight
