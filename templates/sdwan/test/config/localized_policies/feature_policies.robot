...
Get Feature Policies
    ${r}=    GET On Session    sdwan_manager    /dataservice/template/policy/vedge
...
Verify Localized Policies Feature Policies {{ feature_policy.name }}
    ${policy_id}=    Get Value From Json    ${r.json()}    $..data[?(@.policyName=="{{feature_policy.name }}")].policyId
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $.policyDefinition.settings.appVisibilityIPv6    {{ feature_policy.ipv6_application_visibility | default("True") }}    msg=ipv6 application visibility
    Should Be Equal Value Json String    ${r_id.json()}    $.policyDefinition.settings.flowVisibilityIPv6    {{ feature_policy.ipv6_flow_visibility | default("True") }}    msg=ipv6 flow visibility
    ...
    ${route_id}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="vedgeRoute")].definitionId
    ...
    FOR    ${id}    IN    @{route_id}
        ${route_det}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/vedgeroute/${id}
        ${route_name}=    Get Value From Json    ${route_det.json()}    $.name
        Append To List    ${temp_res_list}    ${route_name[0]}
    END
    Lists Should Be Equal    ${temp_res_list}    ${exp_route_policies}    ignore_order=True    msg=route policies

    ${temp_res_list}=    Create List
    ${acl_id}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="acl")].definitionId
    ...
    FOR    ${id}    IN    @{acl_id}
        ${acl_det}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/acl/${id}
        ${acl_name}=    Get Value From Json    ${acl_det.json()}    $.name
        Append To List    ${temp_res_list}    ${acl_name[0]}
    END
    Lists Should Be Equal    ${temp_res_list}    ${exp_acl_list}    ignore_order=True    msg=ipv4 access control lists

    ${temp_res_list}=    Create List
    ${device_acl_id}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="deviceAccessPolicy")].definitionId
    ...
    FOR    ${id}    IN    @{device_acl_id}
        ${device_acl_det}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/deviceaccesspolicy/${id}
        ${device_acl_name}=    Get Value From Json    ${device_acl_det.json()}    $.name
        Append To List    ${temp_res_list}    ${device_acl_name[0]}
    END
    Lists Should Be Equal    ${temp_res_list}    ${exp_device_acl_list}    ignore_order=True    msg=ipv4 device access policies

    ${temp_res_list}=    Create List
    ${ipv6_acl_id}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="aclv6")].definitionId
    ...
    FOR    ${id}    IN    @{ipv6_acl_id}
        ${ipv6_acl_det}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/aclv6/${id}
        ${ipv6_acl_name}=    Get Value From Json    ${ipv6_acl_det.json()}    $.name
        Append To List    ${temp_res_list}    ${ipv6_acl_name[0]}
    END
    Lists Should Be Equal    ${temp_res_list}    ${exp_ipv6_acl_list}    ignore_order=True    msg=ipv6 access control lists

    ${temp_res_list}=    Create List
    ${ipv6_device_acl_id}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="deviceAccessPolicyv6")].definitionId
    ...
    FOR    ${id}    IN    @{ipv6_device_acl_id}
        ${ipv6_device_acl_det}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/deviceaccesspolicyv6/${id}
        ${ipv6_device_acl_name}=    Get Value From Json    ${ipv6_device_acl_det.json()}    $.name
        Append To List    ${temp_res_list}    ${ipv6_device_acl_name[0]}
    END
    Lists Should Be Equal    ${temp_res_list}    ${exp_ipv6_device_acl_list}    ignore_order=True    msg=ipv6 device access policies

    ${temp_res_list}=    Create List
    ${rewrite_id}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="rewriteRule")].definitionId
    ...
    FOR    ${id}    IN    @{rewrite_id}
        ${rewrite_det}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/rewriterule/${id}
        ${rewrite_name}=    Get Value From Json    ${rewrite_det.json()}    $.name
        Append To List    ${temp_res_list}    ${rewrite_name[0]}
    END
    Lists Should Be Equal    ${temp_res_list}    ${exp_rewrite_list}    ignore_order=True    msg=rewrite rules

    ${temp_res_list}=    Create List
    ${qos_id}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="qosMap")].definitionId
    ...
    FOR    ${id}    IN    @{qos_id}
        ${qos_det}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/qosmap/${id}
        ${qos_name}=    Get Value From Json    ${qos_det.json()}    $.name
        Append To List    ${temp_res_list}    ${qos_name[0]}
    END
