...
Verify Feature Profiles System Profiles {{ profile.name }} Performance Monitoring Feature {{ profile.performance_monitoring.name | default(defaults.sdwan.feature_profiles.system_profiles.performance_monitoring.name) }}
    ...
    Run Keyword If    ${system_perfmonitor} == []    Fail    Feature '{{ profile.performance_monitoring.name | default(defaults.sdwan.feature_profiles.system_profiles.performance_monitoring.name) }}}' expected to be configured within the system profile '{{profile.name}}' on the Manager
