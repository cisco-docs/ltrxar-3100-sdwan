...
Verify Feature Profiles System Profiles {{ profile.name }} Performance Monitoring Feature {{ profile.performance_monitoring.name }}
    ...
    Should Be Equal Value Json String    ${system_perfmonitor[0]}    $..name    {{ profile.performance_monitoring.name | default((defaults.sdwan.feature_profiles.system_profiles.performance_monitoring.name) if defaults is defined else "not_defined") }}    msg=name
