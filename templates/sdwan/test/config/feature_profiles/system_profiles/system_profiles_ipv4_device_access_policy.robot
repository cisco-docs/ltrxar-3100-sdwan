...
{% for sequence in profile.ipv4_device_access_policy.sequences | default([]) %}
    ...
    Should Be Equal Value Json Yaml    ${system_ipv4_device_access_policy[0]}    $.data.sequences[{{ loop.index0 }}].matchEntries.destinationDataPrefix.destinationIpPrefixList    ${destination_data_prefixes_list}    {{ sequence.match_entries.destination_data_prefixes_variable | default('not_defined') }}    msg=destination data prefixes    var_msg=destination data prefixes variable

    Should Be Equal Value Json Yaml    ${system_ipv4_device_access_policy[0]}    $.data.sequences[{{ loop.index0 }}].matchEntries.destinationPort    {{ sequence.match_entries.destination_port | default('not_defined') }}    not_defined    msg=destination port    var_msg=not_defined
    ...
    Should Be Equal Value Json Yaml    ${system_ipv4_device_access_policy[0]}    $.data.sequences[{{ loop.index0 }}].matchEntries.sourceDataPrefix.sourceIpPrefixList    ${source_data_prefixes_list}    {{ sequence.match_entries.source_data_prefixes_variable | default('not_defined') }}    msg=source data prefixes    var_msg=source data prefixes variable
    ...
    Should Be Equal Value Json Yaml    ${system_ipv4_device_access_policy[0]}    $.data.sequences[{{ loop.index0 }}].matchEntries.sourcePorts    ${source_ports_list}    not_defined    msg=source ports    var_msg=not_defined

    # Extract refID of the destination data prefix list
    ${refid_destination_data_prefix_list_raw}=    Get Value From Json    ${system_ipv4_device_access_policy[0]}    $.data.sequences[{{ loop.index0 }}].matchEntries.destinationDataPrefix.destinationDataPrefixList.refId.value
    ...
    ${refid_source_data_prefix_list_raw}=    Get Value From Json    ${system_ipv4_device_access_policy[0]}    $.data.sequences[{{ loop.index0 }}].matchEntries.sourceDataPrefix.sourceDataPrefixList.refId.value
