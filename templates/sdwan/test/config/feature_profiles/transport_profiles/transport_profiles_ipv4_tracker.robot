...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv4 Tracker Feature
    ...
    Run Keyword If    ${transport_ipv4_tracker} == []    Fail    Feature '{{profile.ipv4_trackers.name}}' expected to be configured within the transport profile '{{profile.name}}' on the Manager
...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv4 Tracker Group Feature
    ...
    Run Keyword If    ${transport_tracker_grp} == []    Fail    Feature '{{profile.ipv4_tracker_groups.name}}' expected to be configured within the transport profile '{{profile.name}}' on the Manager
    Set Suite Variable    ${transport_tracker_grp}

{% for tracker_grp in profile.ipv4_tracker_groups %}
    ...
    Should Be Equal Value Json String     ${json_tracker_grp}   $..trackerName    {{ tracker_grp.tracker_name | default('not_defined') }}   msg=tracker_grp tracker_name
