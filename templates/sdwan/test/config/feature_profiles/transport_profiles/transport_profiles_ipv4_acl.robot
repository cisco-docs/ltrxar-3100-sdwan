...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv4 ACL Feature
    ...
    ${transport_ipv4_acl_res}=    GET On Session    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id[0]}/ipv4-acl
    Set Suite Variable    ${transport_ipv4_acl_res}
    ${transport_acl}=    Get Value From Json    ${transport_ipv4_acl_res.json()}    $..payload
...
{% for acl in profile.ipv4_acls | default([]) %}
    ...
    ${transport_ipv4_acls_{{ acl.name }}_raw}=    Get Value From Json    ${transport_acl}    $[?(@.name=='{{ acl.name }}')]
    ${transport_ipv4_acls_{{ acl.name }}}=    Set Variable If    ${transport_ipv4_acls_{{ acl.name }}_raw} == []    not_defined    ${transport_ipv4_acls_{{ acl.name }}_raw[0]}

    Should Be Equal Value Json String    ${transport_ipv4_acls_{{ acl.name }}}    $..name    {{ acl.name }}    msg=name
    Should Be Equal Value Json Special_String    ${transport_ipv4_acls_{{ acl.name }}}    $..description    {{ acl.description | default('not_defined') | normalize_special_string }}    msg=description

    Should Be Equal Value Json Yaml    ${transport_ipv4_acls_{{ acl.name }}}    $..data.defaultAction    {{ acl.default_action }}    not_defined    msg=acl.default_action    var_msg=not_defined

    Should Be Equal Value Json List Length    ${transport_ipv4_acls_{{ acl.name }}}    $..sequences    {{ acl.get('sequences', []) | length }}    msg=sequences length
{% if acl.get('sequences', []) | length > 0 %}
    Log    === Sequences List ===
{% for sequence in acl.sequences | default([]) %}
    Log    === Sequence {{ sequence.id }} ===
    ${sequence_raw}=    Get Value From Json    ${transport_ipv4_acls_{{ acl.name }}}    $.data.sequences[?(@.sequenceId.value=={{ sequence.id }})]
    ...
    Should Be Equal Value Json Yaml    ${sequence}    $.sequenceName    {{ sequence.name | default(defaults.sdwan.feature_profiles.transport_profiles.ipv4_acls.sequences.name ) }}    not_defined    msg=acl.sequence.name    var_msg=not_defined
