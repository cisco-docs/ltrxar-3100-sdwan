...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv6 Tracker Feature
    ...
    ${transport_ipv6_tracker_res}=    GET On Session    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id[0]}/ipv6-tracker
    Set Suite Variable    ${transport_ipv6_tracker_res}
    ${transport_ipv6_tracker}=    Get Value From Json    ${transport_ipv6_tracker_res.json()}    $..payload
...
{% for tracker in profile.ipv6_trackers %}
    ...
    ${transport_trackers_{{ tracker.name }}_raw}=    Get Value From Json    ${transport_ipv6_tracker}    $[?(@.name=='{{ tracker.name }}')]
...
Verify Feature Profiles Transport Profiles {{ profile.name }} IPv6 Tracker Group Feature
    Run Keyword If    ${profile} == []    Fail    Feature Profile '{{profile.name}}' should be present on the Manager
    ${transport_tracker_grp_res}=    GET On Session    sdwan_manager    /dataservice/v1/feature-profile/sdwan/transport/${profile_id[0]}/ipv6-trackergroup
...
{% for tracker_grp in profile.ipv6_tracker_groups %}
    ...
    Should Be Equal Value Json Yaml    ${json_tracker_grp}    $..trackerGroupName    {{ tracker_grp.tracker_name | default('not_defined') }}    {{ tracker_grp.tracker_name_variable | default('not_defined') }}    msg=tracker_grp name    var_msg=tracker_grp name_variable
