...
{% endfor %}
    ...
    ${expected_traffic_class}=    Set Variable If    {{ sequence.get('match_entries', {}).get('traffic_class', []) | length == 0 }}    not_defined    {{ sequence.get('match_entries', {}).get('traffic_class', []) }}
    Should Be Equal Value Json Yaml    ${sequence}    $.matchEntries[0].trafficClass    ${expected_traffic_class}    not_defined    msg=acl.sequence.match_entries.trafficClass    var_msg=not_defined
    Log    ${sequence}
