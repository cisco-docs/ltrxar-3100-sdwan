...
Verify Feature Profiles Service Profiles {{ profile.name }} Route Policy Feature
    ...
    ${service_route_policy_res}=    GET On Session    sdwan_manager    /dataservice/v1/feature-profile/sdwan/service/${profile_id[0]}/route-policy
    Set Suite Variable    ${service_route_policy_res}
    ${service_route_policy}=    Get Value From Json    ${service_route_policy_res.json()}    $..payload
...
{% for route_policy in profile.route_policies | default([]) %}
    ...
    ${service_route_policies_{{ route_policy.name }}_raw}=    Get Value From Json    ${service_route_policy}    $[?(@.name=='{{ route_policy.name }}')]
    ${service_route_policies_{{ route_policy.name }}}=    Set Variable If    ${service_route_policies_{{ route_policy.name }}_raw} == []    not_defined    ${service_route_policies_{{ route_policy.name }}_raw[0]}
...
{% for sequence in route_policy.sequences | default([]) %}
    ...
    ${sequence_raw}=    Get Value From Json    ${service_route_policies_{{ route_policy.name }}}    $.data.sequences[?(@.sequenceId.value=={{ sequence.id }})]
    ...
    Should Be Equal Value Json Yaml    ${sequence}    $.sequenceName    {{ sequence.name | default(defaults.sdwan.feature_profiles.service_profiles.route_policies.sequences.name ) }}    not_defined    msg=route_policy.sequence.name    var_msg=not_defined
