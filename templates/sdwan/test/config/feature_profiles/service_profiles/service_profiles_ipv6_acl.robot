...
Verify Feature Profiles Service Profiles {{ profile.name }} IPv6 ACL Feature
    ...
    ${service_ipv6_acl_res}=    GET On Session    sdwan_manager    /dataservice/v1/feature-profile/sdwan/service/${profile_id[0]}/ipv6-acl
    Set Suite Variable    ${service_ipv6_acl_res}
    ${service_acl}=    Get Value From Json    ${service_ipv6_acl_res.json()}    $..payload
...
{% for acl in profile.ipv6_acls | default([]) %}
    ...
    ${service_ipv6_acls_{{ acl.name }}_raw}=    Get Value From Json    ${service_acl}    $[?(@.name=='{{ acl.name }}')]
    ${service_ipv6_acls_{{ acl.name }}}=    Set Variable If    ${service_ipv6_acls_{{ acl.name }}_raw} == []    not_defined    ${service_ipv6_acls_{{ acl.name }}_raw}
    Log    ${service_ipv6_acls_{{ acl.name }}}

    Should Be Equal Value Json String    ${service_ipv6_acls_{{ acl.name }}}    $..name    {{ acl.name }}    msg=name
    Should Be Equal Value Json Special_String    ${service_ipv6_acls_{{ acl.name }}}    $..description    {{ acl.description | default('not_defined') | normalize_special_string }}    msg=description

    Should Be Equal Value Json String    ${service_ipv6_acls_{{ acl.name }}}    $..data.defaultAction.value    {{ acl.default_action }}    msg=acl.default_action

    Should Be Equal Value Json List Length    ${service_ipv6_acls_{{ acl.name }}}    $..sequences    {{ acl.get('sequences', []) | length }}    msg=sequences length
{% if acl.get('sequences', []) | length > 0 %}
    Log    === Sequences List ===
{% for sequence in acl.sequences | default([]) %}
    Log    === Sequence {{ sequence.id }} ===
    ${sequence_raw}=    Get Value From Json    ${service_ipv6_acls_{{ acl.name }}}    $..data.sequences[?(@.sequenceId.value=={{ sequence.id }})]
    ...
    Should Be Equal Value Json Yaml    ${sequence}    $.sequenceName    {{ sequence.name | default(defaults.sdwan.feature_profiles.service_profiles.ipv6_acls.sequences.name ) }}    not_defined    msg=acl.sequence.name    var_msg=not_defined
...
{% endfor %}
    ...
    Should Be Equal Value Json Yaml    ${sequence}    $.matchEntries[0].icmp6Msg    {{ sequence.match_entries.icmp6_messages | default('not_defined') }}    not_defined    msg=acl.sequence.match_entries.icmp6_messages    var_msg=not_defined
