...
{% endif %}
    ...
    ${eigrp_raw}=    Get Value From Json    ${service_profile_features[0]}    $[?(@.payload.name=='${service_lan_vpn_data['name']}')].subparcels[?(@.parcelType=='routing/eigrp')]
    ${eigrp}=    Set Variable If    ${eigrp_raw} == []    not_defined    ${eigrp_raw[0]}
    Should Be Equal Value Json String    ${eigrp}    $.payload.name   {{ lan_vpn.eigrp | default('not_defined') }}     msg=service lan vpn eigrp
