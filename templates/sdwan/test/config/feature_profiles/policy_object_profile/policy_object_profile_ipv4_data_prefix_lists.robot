...
{% if sdwan.feature_profiles.policy_object_profile.ipv4_data_prefix_lists is defined %}
...
Get IPv4 Data Prefix Lists
    ...
    ${ipv4_data_prefix_raw}=    GET On Session    sdwan_manager    /dataservice/v1/feature-profile/sdwan/policy-object/${profile_id[0]}/data-prefix
...
Verify Feature Profiles Policy Object Profile {{ sdwan.feature_profiles.policy_object_profile.name }} IPv4 Data Prefix List Feature {{ ipv4_data_prefix_list.name }}
    ...
    ${ipv4_data_prefix_lists}=    Get Value From Json    ${ipv4_data_prefix_raw.json()}    $..data[?(@..name=='{{ ipv4_data_prefix_list.name }}')]..payload
    Run Keyword If    ${ipv4_data_prefix_lists} == []    Fail    Feature '{{ ipv4_data_prefix_list.name }}' expected to be configured within the policy object profile '{{ sdwan.feature_profiles.policy_object_profile.name }}' on the Manager

    Should Be Equal Value Json String    ${ipv4_data_prefix_lists[0]}    $..name    {{ ipv4_data_prefix_list.name }}    msg=name
    Should Be Equal Value Json Special_String     ${ipv4_data_prefix_lists[0]}     $.description    {{ ipv4_data_prefix_list.description | default('not_defined') | normalize_special_string }}    msg=description

    ${prefixes_list}=    Create List    {{ ipv4_data_prefix_list.get('prefixes', []) | join('   ') }}
    # Extract features from the JSON
    ${prefixes_list_json}=    Evaluate    [f"{item['ipv4Address']['value']}/{item['ipv4PrefixLength']['value']}" for item in ${ipv4_data_prefix_lists[0]['data']['entries']} if 'ipv4Address' in item and 'ipv4PrefixLength' in item]
