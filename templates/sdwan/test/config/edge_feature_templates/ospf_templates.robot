...
Verify Edge Feature Template OSPF Feature Template {{ ft_yaml.name }}
    ...
    ${default_information_originate}=   Get Value From Json   ${ft.json()}        $.ospf..["default-information"].originate..vipType
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.ospf..["route-policy"]..["pol-name"]
...
{% for area_index in range(ft_yaml.areas | default([]) | length()) %}
    ...
    ${rec_area_type}=    Get Value From Json    ${ft.json()}    $.ospf.area.vipValue[{{ area_index }}]
