...
Verify Edge Feature Template Secure Internet Gateway Feature Template {{ ft_yaml.name }}
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["umbrella-data-center"]..["data-center-primary"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["umbrella-data-center"]..["data-center-secondary"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].aup..["block-internet-until-accepted"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].aup.enabled
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].aup..["force-ssl-inspection"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].aup.timeout
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"]..["auth-required"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"]..["caution-enabled"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"]..["ips-control"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"]..["ofw-enabled"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"]..["location-name"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].datacenters..["primary-data-center"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].datacenters..["secondary-data-center"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].surrogate..["display-time-unit"]
    ...    {{ display_time_unit }}
    ...    {{ ft_yaml.zscaler_surrogate_display_time_unit_variable | default("not_defined") }}
    ...    msg=zscaler_surrogate_display_time_unit
    # End of custom handling

    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].surrogate..["idle-time"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].surrogate.ip
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].surrogate..["ip-enforced-for-known-browsers"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].surrogate..["refresh-time"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"].surrogate..["refresh-time-unit"]
    ...    {{ refresh_time_unit }}
    ...    {{ ft_yaml.zscaler_surrogate_refresh_time_unit_variable | default("not_defined") }}
    ...    msg=zscaler_surrogate_refresh_time_unit
    # End of custom handling

    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["zscaler-location-settings"]..["xff-forward-enabled"]
    ...
    Should Be Equal Value Json List Length    ${ft.json()}    $.service.vipValue..["ha-pairs"]..["interface-pair"].vipValue    {{ ft_yaml.high_availability_interface_pairs | default([]) | length }}    msg=high_availability_interface_pairs length

{% for ha_pairs in ft_yaml.high_availability_interface_pairs | default([]) %}

    Log    === High Availability Interface Pair {{loop.index0}} ===
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["ha-pairs"]..["interface-pair"].vipValue[{{loop.index0}}]..["active-interface"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["ha-pairs"]..["interface-pair"].vipValue[{{loop.index0}}]..["active-interface-weight"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["ha-pairs"]..["interface-pair"].vipValue[{{loop.index0}}]..["backup-interface"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.service.vipValue..["ha-pairs"]..["interface-pair"].vipValue[{{loop.index0}}]..["backup-interface-weight"]
