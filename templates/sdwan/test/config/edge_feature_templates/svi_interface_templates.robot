...
Verify Edge Feature Template SVI Interface Template {{ ft_yaml.name }}
...
{% for ipv6_vrrp_index in range(ft_yaml.ipv6_vrrp_groups | default([]) | length()) %}
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $["ipv6-vrrp"].vipValue[{{ ipv6_vrrp_index }}].ipv6.vipValue..prefix
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $["ipv6-vrrp"].vipValue[{{ ipv6_vrrp_index }}].ipv6.vipValue..["ipv6-link-local"]
