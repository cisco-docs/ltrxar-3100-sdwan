...
Verify Edge Feature Template Cellular Interface Feature Template {{ ft_yaml.name }}
    ...
    ${nat_json}=    Get Value From Json    ${ft.json()}    $..nat..vipType
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.policer.vipValue[?(@.direction.vipValue=="in")]..["policer-name"]
    ...
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $.policer.vipValue[?(@.direction.vipValue=="out")]..["policer-name"]
