...
{% for address_family in neighbor.address_families | default([]) %}
    ...
    Should Be Equal Value Json String    ${r_id.json()}    $.[bgp].{{neighbor1}}.vipValue[{{loop.index0}}].address-family.vipValue[{{loop.index0}}].route-policy.vipValue[?(@.direction.vipValue=="out")]..["pol-name"].vipVariableName    {{ address_family.route_policy_out_variable | default("not_defined") }}    msg=address family family route policy out variable
