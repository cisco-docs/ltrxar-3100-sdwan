...
Verify Edge Feature Template System Feature Template {{ ft_yaml.name }}
    ...
    Should Be Equal Value Json List Length    ${ft.json()}    $..["geo-fencing"].sms.mobile-number.vipValue    {{ ft_yaml.geo_fencing_sms_phone_numbers | default([]) | length }}    msg=geo_fencing_sms_phone_numbers.length

{% for geo_fencing_phone in ft_yaml.geo_fencing_sms_phone_numbers | default([]) %}

    Log    === Geo Fencing SMS Phone Number {{loop.index0}} ===
    Should Be Equal Value Json Yaml UX1    ${ft.json()}    $..["geo-fencing"].sms.mobile-number.vipValue[{{loop.index0}}].number
