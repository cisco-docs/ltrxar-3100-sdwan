...
{% set type = ({"qos":"qos", "service_chaining":"serviceChaining", "traffic_engineering":"trafficEngineering", "application_firewall":"applicationFirewall", "custom":"data"}) %}
    ...
    IF    ${app_list_id} == []
        Should Be Equal Value Json String    $..sequences[{{loop.index0}}].match.entries[?(@..field=="appList")].ref    {{ item.match_criterias.application_list | default("not_defined") }}    msg=application list
    ELSE
        ${r_app_list_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/list/app/${app_list_id[0]}
    Set Suite Variable    ${r_app_list_id}
...
{% else %}
    Should Be Equal Value Json String     ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@..field=="destinationPort")].value    {{ item.match_criterias.destination_ports | default("not_defined") }}    msg=destination ports are not defined
    Should Be Equal Value Json String     ${r_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@..field=="destinationPort")].value    {{ item.match_criterias.destination_port_ranges | default("not_defined") }}    msg=destination port ranges are not defined
{% endif %}
...
{% if item.type == "custom" %}
    ...
    IF    ${dns_app_list_id} == []
        Should Be Equal Value Json String    ${r_dns_app_list_id.json()}    $..sequences[{{loop.index0}}].match.entries[?(@..field=="dnsAppList")].ref    {{ item.match_criterias.dns_application_list | default("not_defined") }}    msg=dns application list
    ELSE
