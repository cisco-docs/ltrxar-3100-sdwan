...
Verify Centralized Policies Feature Policies {{ fp.name }}
    ...
    ${hst_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/hubandspoke/${hs_ids_list[{{ hs_index }}]}
    ${hst_name}=    Get Value From Json    ${hst_id.json()}    $..name
    ${hs_dict}[${hst_name[0]}]=    Set Variable    ${hs_ids_list[{{ hs_index }}]}
    {% endfor %}

{% for fp_hub_and_spoke in fp.hub_and_spoke_topology | default([]) %}
    ${fp_hub_and_spoke_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/hubandspoke/${hs_dict["{{ fp_hub_and_spoke.policy_definition }}"]}
...
{% endfor %}
    ...
    ${mesh_name}=    Get Value From Json    ${mesh_id.json()}    $..name
    ${mesh_dict}[${mesh_name[0]}]=    Set Variable    ${mesh_ids_list[{{ mesh_index }}]}
    {% endfor %}

{% for fp_mesh in fp.mesh_topology | default([]) %}
    ${mesh_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/mesh/${mesh_dict["{{ fp_mesh.policy_definition }}"]}
...
{% endfor %}
    ...
    ${vpn_name}=    Get Value From Json    ${vpn_id.json()}    $..name
    ${vpn_dict}[${vpn_name[0]}]=    Set Variable    ${vpn_ids_list[{{ vpn_index }}]}
    {% endfor %}

{% for fp_vpn in fp.vpn_membership | default([]) %}
    ${vpn_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/vpnmembershipgroup/${vpn_dict["{{ fp_vpn.policy_definition }}"]}
...
{% endfor %}
    ...
    ${cflowd_name}=    Get Value From Json    ${cflowd_id.json()}    $..name
    ${cflowd_dict}[${cflowd_name[0]}]=    Set Variable    ${cflowd_ids_list[{{ cflowd_index }}]}
    {% endfor %}

{% for cflowd in fp.cflowd | default([]) %}
    ${cflowd_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/cflowd/${cflowd_dict["{{ cflowd.policy_definition }}"]}
    Should Be Equal Value Json String    ${cflowd_id.json()}    $..name    {{ cflowd.policy_definition }}    msg=cflowd name

    # cflowd Site List
    ${cflowd_site_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="cflowd" & @.definitionId=="${cflowd_dict["{{ cflowd.policy_definition }}"]}")].entries[*].siteLists
    IF    ${cflowd_site_list} == []
        ${cflowd_site_list_length}=    Set Variable    0
        ${cflowd_site_list}=    Create List
    ELSE
        ${cflowd_site_list_length} =    Get Length    ${cflowd_site_list[0]}
        ${cflowd_site_list}=    Set Variable    ${cflowd_site_list[0]}
    END
    ...
    FOR    ${id}    IN    @{cflowd_site_list}
        ...
        ${site_list_id_name}=    Get Value From Json    ${site_list_id.json()}    $.name
        Append To List    ${site_list_name}    ${site_list_id_name[0]}
    END
    Log    ${site_list_name}
...
{% endfor %}
    ...
    ${cctt_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/control/${cct_ids_list[{{ cct_index }}]}
    ${cct_name}=    Get Value From Json    ${cctt_id.json()}    $..name
    ${cct_dict}[${cct_name[0]}]=    Set Variable    ${cct_ids_list[{{ cct_index }}]}
    {% endfor %}

{% for cct_policy in fp.custom_control_topology | default([]) %}
    ${cct_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/control/${cct_dict["{{ cct_policy.policy_definition }}"]}
    ...
    ${cct_sr_sl_id_in_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="control" & @.definitionId=="${cct_dict["{{ cct_policy.policy_definition }}"]}")].entries[?(@.direction=="in")].siteLists
        IF    ${cct_sr_sl_id_in_list} == []
            ${cct_sr_sl_id_in_list_length}=    Set Variable    0
            ${cct_sr_sl_id_in_list}=    Create List
        ELSE
    ...
    ${cct_sr_sl_id_out_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="control" & @.definitionId=="${cct_dict["{{ cct_policy.policy_definition }}"]}")].entries[?(@.direction=="out")].siteLists
        IF    ${cct_sr_sl_id_out_list} == []
            ${cct_sr_sl_id_out_list_length}=    Set Variable    0
            ${cct_sr_sl_id_out_list}=    Create List
        ELSE
    ...
    ${cct_sr_rl_id_in_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="control" & @.definitionId=="${cct_dict["{{ cct_policy.policy_definition }}"]}")].entries[?(@.direction=="in")].regionLists
        IF    ${cct_sr_rl_id_in_list} == []
            ${cct_sr_rl_id_in_list_length}=    Set Variable    0
            ${cct_sr_rl_id_in_list}=    Create List
        ELSE
    ...
    ${cct_sr_rl_id_out_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="control" & @.definitionId=="${cct_dict["{{ cct_policy.policy_definition }}"]}")].entries[?(@.direction=="out")].regionLists
        IF    ${cct_sr_rl_id_out_list} == []
            ${cct_sr_rl_id_out_list_length}=    Set Variable    0
            ${cct_sr_rl_id_out_list}=    Create List
        ELSE
...
{% endfor %}
    ...
    ${aar_ids_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="appRoute")].definitionId
    ...
    ${aar_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/approute/${aar_ids_list[{{ aar_index }}]}
    ${aar_name}=    Get Value From Json    ${aar_id.json()}    $..name
    ${aar_dict}[${aar_name[0]}]=    Set Variable    ${aar_ids_list[{{ aar_index }}]}
    {% endfor %}

{% for fp_app_route in fp.application_aware_routing | default([]) %}
    ${aar_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/approute/${aar_dict["{{ fp_app_route.policy_definition }}"]}
    ...
    ${aar_sl_id_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="appRoute" & @.definitionId=="${aar_dict["{{ fp_app_route.policy_definition }}"]}")].entries[*].siteLists
        IF    ${aar_sl_id_list} == []
    ...
    ${aar_rl_id_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="appRoute" & @.definitionId=="${aar_dict["{{ fp_app_route.policy_definition }}"]}")].entries[*].regionLists
        IF    ${aar_rl_id_list} == []
            ${aar_rl_id_list_length}=    Set Variable    0
            ${aar_rl_id_list}=    Create List
        ELSE
            ${aar_rl_id_list_length} =    Get Length    ${aar_rl_id_list[0]}
            ${aar_rl_id_list}=    Set Variable    ${aar_rl_id_list[0]}
        END
    ${aar_rl_id_name_list}=    Create List
    FOR    ${id}    IN    @{aar_rl_id_list}
        ${region_list_id}=   GET On Session   sdwan_manager   /dataservice/template/policy/list/region/${id}
    ...
    ${aar_vl_id_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="appRoute" & @.definitionId=="${aar_dict["{{ fp_app_route.policy_definition }}"]}")].entries[*].vpnLists
        IF    ${aar_vl_id_list} == []
...
{% endfor %}
    ...
    ${td_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/data/${td_ids_list[{{ td_index }}]}
    ${td_name}=    Get Value From Json    ${td_id.json()}    $..name
    ${td_dict}[${td_name[0]}]=    Set Variable    ${td_ids_list[{{ td_index }}]}
    {% endfor %}

{% for fp_traffic_data in fp.traffic_data | default([]) %}
    ${td_id}=    GET On Session    sdwan_manager    /dataservice/template/policy/definition/data/${td_dict["{{ fp_traffic_data.policy_definition }}"]}
...
{% for srv in fp_traffic_data.site_region_vpn | default([]) %}
    ...
    ${td_sr_sl_id_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="data" & @.definitionId=="${td_dict["{{ fp_traffic_data.policy_definition }}"]}")].entries[?(@.direction=="{{ srv.direction }}")].siteLists
        IF    ${td_sr_sl_id_list} == []
            ${td_sr_sl_id_list_length}=    Set Variable    0
            ${td_sr_sl_id_list}=    Create List
        ELSE
            ${td_sr_sl_id_list_length} =    Get Length    ${td_sr_sl_id_list[0]}
            ${td_sr_sl_id_list}=    Set Variable    ${td_sr_sl_id_list[0]}
        END
    ...
    FOR    ${id}    IN    @{td_sr_sl_id_list}
        ${site_list_id}=   GET On Session   sdwan_manager   /dataservice/template/policy/list/site/${id}
        ${site_list_name}=    Get Value From Json    ${site_list_id.json()}    $.name
        Append To List    ${td_sr_sl_id_name_list}    ${site_list_name[0]}
    END
    ...
    ${td_sr_vl_id_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="data" & @.definitionId=="${td_dict["{{ fp_traffic_data.policy_definition }}"]}")].entries[?(@.direction=="{{ srv.direction }}")].vpnLists
        IF    ${td_sr_vl_id_list} == []
            ${td_sr_vl_id_list_length}=    Set Variable    0
            ${td_sr_vl_id_list}=    Create List
        ELSE
            ${td_sr_vl_id_list_length} =    Get Length    ${td_sr_vl_id_list[0]}
            ${td_sr_vl_id_list}=    Set Variable    ${td_sr_vl_id_list[0]}
        END
    ...
    FOR    ${id}    IN    @{td_sr_vl_id_list}
        ${vpn_list_id}=   GET On Session   sdwan_manager   /dataservice/template/policy/list/vpn/${id}
        ${vpn_list_name}=    Get Value From Json    ${vpn_list_id.json()}    $.name
        Append To List    ${td_sr_vl_id_name_list}    ${vpn_list_name[0]}
    END
    ...
    ${td_sr_rl_id_list}=    Get Value From Json    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="data" & @.definitionId=="${td_dict["{{ fp_traffic_data.policy_definition }}"]}")].entries[?(@.direction=="{{ srv.direction }}")].regionLists
        IF    ${td_sr_rl_id_list} == []
            ${td_sr_rl_id_list_length}=    Set Variable    0
            ${td_sr_rl_id_list}=    Create List
        ELSE
            ${td_sr_rl_id_list_length} =    Get Length    ${td_sr_rl_id_list[0]}
            ${td_sr_rl_id_list}=    Set Variable    ${td_sr_rl_id_list[0]}
        END
    ...
    # traffic data region
    Should Be Equal Value Json String    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="data" & @.definitionId=="${td_dict["{{ fp_traffic_data.policy_definition }}"]}")].entries[?(@.direction=="{{ srv.direction }}")].regionIds[0]    {{ srv.region | default("not_defined") }}    msg=traffic data region for {{ fp_traffic_data.policy_definition }} and direction {{ srv.direction }}
    
    # traffic data direction
    Should Be Equal Value Json String    ${r_id.json()}    $.policyDefinition.assembly[?(@.type=="data" & @.definitionId=="${td_dict["{{ fp_traffic_data.policy_definition }}"]}")].entries[?(@.direction=="{{ srv.direction }}")].direction    {{ srv.direction }}    msg=traffic data direction for {{ fp_traffic_data.policy_definition }} and direction {{ srv.direction }}
