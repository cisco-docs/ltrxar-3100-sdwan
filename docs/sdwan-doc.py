...
OBJECTS_PATH = "./objects/sdwan.yaml"
DEFAULTS_PATH = "./defaults/sdwan.yaml"
...
def render_diagram_class(schema, class_path, paths, rendered_paths, mappings):
    output = ""
    for path in paths:
        path_elements = path.split(".")
        cp = ".".join(path_elements[:-1])
        if class_path == cp:
            ...
            for idx, path_element in enumerate(path_elements):
                ...
                element, _ = read_schema_path(schema, path_string)
...
def render_diagram(schema, defaults, class_paths, paths):
    ...
    for class_path in ordered_class_paths:
        output += render_diagram_class(
            schema, class_path, paths, rendered_paths, mappings
        )
    output += "```\n"
    output += "</figure>\n\n"
    return output
...
def main():
    render_doc("sdwan", SCHEMA_PATH, OBJECTS_PATH, DEFAULTS_PATH)
